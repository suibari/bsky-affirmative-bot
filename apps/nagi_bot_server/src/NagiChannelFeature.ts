...
const welcomePrompt = (name: string, description: string | null) =>
  `あなたはSNS「Nagi」のマスコット「botたん」です。新しくチャンネル「${name}」が作られました。` +
  (description ? `チャンネルの説明は「${description}」です。` : "") +
  `このチャンネルの最初の投稿として、テーマに沿ってみんなが気軽に投稿したくなるような、` +
  `やさしくて短い歓迎・盛り上げのメッセージを1つだけ日本語で書いてください。` +
  `説明文や鉤括弧の注釈は不要、投稿本文だけを140文字以内で。`;

const topicPrompt = (name: string, description: string | null) =>
  `あなたはSNS「Nagi」のマスコット「botたん」です。しばらく投稿の途絶えたチャンネル「${name}」を、そっと盛り上げたいです。` +
  (description ? `チャンネルの説明は「${description}」です。` : "") +
  `このチャンネルのテーマに沿った、みんなが答えたくなる軽いお題や話題ふりを1つだけ日本語で書いてください。` +
