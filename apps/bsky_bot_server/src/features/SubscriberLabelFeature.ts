...
/**
 * Synchronizes the "bot-tan-sub" label between the Google Sheets subscriber list and the Labeler SQLite DB.
 */
export async function syncSubscriberLabels() {
  console.log("[INFO][LABEL-SYNC] Starting subscriber label synchronization...");
  try {
    // 1. Fetch current subscribers from the Google Sheet
    const sheetSubscribers = await getSubscribersFromSheet();
    const sheetSubSet = new Set(sheetSubscribers);
    ...
    console.log(`[INFO][LABEL-SYNC] Current subscribers in sheet: ${sheetSubSet.size}, in SQLite: ${activeLabelSubSet.size}`);

    // 3. Subscribers to add (in sheet but not in SQLite)
    const toAdd = sheetSubscribers.filter(did => !activeLabelSubSet.has(did));

    // 4. Subscribers to remove (in SQLite but not in sheet)
