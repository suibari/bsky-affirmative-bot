...
export async function awardRegularBadge(did: string): Promise<void> {
    ...
    const nextDef = BADGE_DEF.regularLv(nextLevel, levelLabel);
    // 期限切れ後に再訪してもレベルが変わらない場合（例: Lv.1失効 → 再訪でLv.1維持）は
    // prevBadgeId === nextDef.id になる。これは「バッジの交代」ではなく「同じバッジの更新（exp延長）」。
    const prevBadgeId = currentLevel > 0 ? BADGE_DEF.regularLv(currentLevel, '').id : null;
    ...
    await botLabelerManager.applyLabel(did, nextDef.id, false, expDate);

    // 旧バッジの剥奪は、ID が新バッジと異なる場合のみ行う。
    // ラベラーは (uri, val) の組ごとに「最後に書き込まれた行の neg 値」で有効/無効を判定するため
    // （labeler_server の /active-dids は MAX(id) の neg を見ている）、prevBadgeId === nextDef.id
    ...
    // 剥奪自体が不要であり、スキップする。
    if (prevBadgeId && prevBadgeId !== nextDef.id) {
    ...
    console.log(`[INFO][BADGE][REGULAR] Successfully applied badge ${nextDef.id} to ${did} (exp=${expDate})`);
