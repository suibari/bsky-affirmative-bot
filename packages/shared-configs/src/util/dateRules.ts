...
export const dateForHoliday = (year: number, holiday: Holiday): Date => {
  switch (holiday.rule.type) {
    case "fixed":
      return new Date(Date.UTC(year, holiday.rule.month - 1, holiday.rule.day, 0, 0, 0, 0));
    case "nth-weekday":
      return nthWeekdayOfMonthUTC(year, holiday.rule.month, holiday.rule.week, holiday.rule.weekday);
    case "easter":
      return holiday.rule.calendar === "western" ? westernEasterUTC(year) : orthodoxEasterUTC(year);
    default:
      throw new Error("Invalid holiday rule type");
  }
};
