 ...
 *
 * 全文（約4000字）を渡せない場所で使う。具体的には
 * (1) 今期の話題作をグラウンディングで拾うとき（何を拾えばこの子が喜ぶかの基準）
 * (2) BOT_SCENE_BRIEF_JA（ローカルの小型モデルへ渡す描写用ブリーフ）
...
/**
 * ローカルの小型モデル（gemma3:4b 想定）へ渡す、状況描写用のキャラクターブリーフ。
 *
 * SYSTEM_INSTRUCTION 全文は 4b モデルには長すぎてタスクが薄まる（BOT_VOICE_BRIEF_EN と同じ理由）。
 * ここには「いま何をしているか」を書くのに要る要素だけを置く。性格・背景・全肯定の思想は
 * 描写には効かないので入れない。SYSTEM_INSTRUCTION のサマリなので、見た目や友達を変えたら両方直すこと。
 */
