...
import { DIARY_REGISTER_TRIGGER, DIARY_RELEASE_TRIGGER } from "../config";
import { Record } from "@atproto/api/dist/client/types/app/bsky/feed/post";
import { handleMode } from "./utils";
import { getLangStr, getTimezoneFromLang } from "../bsky/util";
...
export class DiaryFeature implements BotFeature {
    ...
    async handle(event: CommitCreateEvent<"app.bsky.feed.post">, follower: ProfileView, context: FeatureContext): Promise<void> {
        const { db } = context;
        if (await this.handleDiaryRegister(event, db)) return;
        if (await this.handleDiaryRelease(event, db)) return;
    }

    private async handleDiaryRegister(event: CommitCreateEvent<"app.bsky.feed.post">, db: SQLite3) {
    ...
    private async handleDiaryRelease(event: CommitCreateEvent<"app.bsky.feed.post">, db: SQLite3) {
        const record = event.commit.record as Record;
        const langStr = getLangStr(record.langs);
 ...
 * @param db SQLite3 instance.
 */
export async function scheduleAllUserDiaries(db: SQLite3) {
