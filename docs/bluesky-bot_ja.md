...
[← 全肯定botたんプロジェクトの総合READMEへ戻る](../README_ja.md) | [English](./bluesky-bot.md)
...
ローカルLLMおよび生成AIを活用し、フォロワーを励ますことを目的とします。
...
2. **定型文リプライ**: 高精度な挨拶ルールで判定し、それ以外をOllama上のGemma 3 4Bでネガティブ・ニュートラル・ポジティブに分類します。分類後はカテゴリ内からランダムに選び、日英以外では英語定型文をローカルLLMで相手言語へ翻訳します。比較結果は[評価記録](./evaluations/predefined-reply/2026-08-11-summary.md)に保存しています
...
定型文リプライの分類は自宅のローカルモデルで行い、Gemini APIは使用しません。
...
定型文リプライにはGemini APIではなく、自宅のローカルモデルを使用します。
...
### 過去バージョンの引用文献
以前の分類方式では、東北大学 乾・岡崎研究室の[日本語評価極性辞書](https://www.cl.ecei.tohoku.ac.jp/Open_Resources-Japanese_Sentiment_Polarity_Dictionary.html)と、東京工業大学 奥村・高村研究室の[単語感情極性対応表](http://www.lr.pi.titech.ac.jp/~takamura/pndic_en.html)を使用していました。
