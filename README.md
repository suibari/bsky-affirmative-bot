...
11. **ステータス確認機能**: botとの付き合い日数や機能のクールダウン状況などを一覧で確認できます
12. **一年のまとめ (Recap) 機能**: 直近1年間のポスト（最大1000件）を収集し、投稿傾向やよく絡む仲良しユーザーを分析した1年の総括（画像付き）をリプライします
13. **バッジ（Bluesky Label）機能**: 特定の条件を満たすことで、Bluesky上のあなたのプロフィールに特別なバッジを付与します
...
| サブスク可否       | 定型文リプライ | AI生成リプライ | 占い | リプ頻度調整 | 会話機能 | 分析機能 | 応援機能 | DJ機能 | 日記機能 | 記念日機能 | ステータス確認 | 一年のまとめ | バッジ機能※1 |
| ----------------- | ------------- | ------------- | ---- | ---------- | -------- | ------- | ------- | ------ | -------- | -------- | -------------- | ------------ | ----------- |
| 通常フォロワー     | ✓             | ✓※2        | ✓   | ✓          |          | ✓      |         | ✓     |          | ✓       | ✓              | ✓            | ✓           |
| サブスクフォロワー |                | ✓           | ✓   | ✓          | ✓       | ✓      | ✓       | ✓     | ✓       | ✓       | ✓              | ✓            | ✓           |

※1: サブスクライバー限定の「サブスクライバーバッジ」を除くバッジが獲得可能です。
※2: 通常フォロワーのAI生成リプライは確率で発生します。
...
### 会話機能
以下の手順を実施することで、そのフォロワーに対して会話を開始します（※サブスクフォロワー限定機能です）。
...
### 応援機能
以下の手順を実施することで、本botがあなたのポストをリポストして応援します（※サブスクフォロワー限定機能です）。
...
### 日記機能
以下の手順を実施することで、本botがあなたの1日のポストから日記をつけます（※サブスクフォロワー限定機能です）。
...
### ステータス確認機能
以下の手順を実施することで、botとの現在のステータスを一覧形式で確認できます。
ステータス確認は1度行うと **8時間** 行えません。

1. "使用方法"に従い、本botからフォローされた状態となる
2. 本botに対しメンションまたはリプライで **"ステータス教えて"** （または "教えてステータス", "おしえてステータス", "ステータスおしえて", "tell me status"）とポストする
3. 本botがあなたとbotの間のステータス情報（付き合い日数、AI/定型文有効設定、リプ頻度、占い/分析/応援のクールダウン状況、会話履歴数、登録記念日）をまとめてリプライします。
...
> プレゼントされたバッジを自身のBlueskyアカウントに表示させるには、事前に以下のラベラーアカウントを購読（Subscribe）しておく必要があります。
>
> 👉 **[botたんラベラー (labeler-bot-tan.suibari.com)](https://bsky.app/profile/labeler-bot-tan.suibari.com)**
>
> リンク先のプロフィールから **「Subscribe (購読)」** ボタンを押すだけで準備完了です！

### 獲得できるバッジ一覧

| バッジ種類 | 識別キー | 獲得条件 | 有効期間 | 表示内容の例 |
| :--- | :--- | :--- | :--- | :--- |
| **サブスクライバー** | `bot-tan-sub` | botたんのサブスク会員になる（Discord連携等） | サブスク期間中 (永続) | `サブスクライバー` |
